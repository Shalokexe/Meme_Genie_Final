/**
 * Meme Genie 🧞‍♂️ - Memer Community Portal Engine (v5.0.0-Beta)
 * "MADE BY MEMERS, MADE FOR MEMERS"
 * 4-Phase App Journey: Landing & Auth -> Meme IQ Quiz -> Spotify Taste Grid -> Instagram Portal.
 */
...
async function handleAccountAuth(e) {
    ...
    // Sync profile and start Phase 2: Meme IQ Quiz
    await syncUserProfile();
    switchAppPhase('iq_quiz');
}

// --- PHASE 2: MEME IQ CALIBRATION QUIZ ---
...
function renderIqQuestion() {
    if (iqCurrentIndex >= iqQuizData.length) {
        finishIqQuiz();
        return;
    }
...
// --- PHASE 3: SPOTIFY-STYLE MEME TASTE SELECTION (B&W TO COLOR) ---
        ...
        container.innerHTML = (data.posts || []).map(post => `
            <div class="fyp-post-card">
                ...
                <div class="media-frame" style="max-height:360px;">
                    <img src="${post.media_url}" alt="${post.meme_name}">
                </div>
...
// --- MEMEX MARKET, HALL OF FAME & STUDIO ---
            ...
            grid.innerHTML = data.stocks.map(s => `
                <div class="crystal-subpanel">
                    ...
                    <div style="display:flex; gap:6px; margin-top:6px;">
                        <button class="crystal-btn" style="flex:1; padding:4px; font-size:12px;" onclick="executeTrade('${s.ticker}', 'buy')">Buy</button>
                        <button class="crystal-btn" style="flex:1; padding:4px; font-size:12px;" onclick="executeTrade('${s.ticker}', 'sell')">Sell</button>
                    </div>
                </div>
            `).join("");
        }
...
async function executeTrade(ticker, action) {
    try {
        ...
        if (grid && data.rankings) {
            grid.innerHTML = data.rankings.map((m, idx) => `
                <div class="crystal-subpanel" style="display:flex; gap:14px; align-items:center;">
                    ...
                    <div class="media-frame" style="max-width:120px; max-height:80px;"><img src="${m.media_url}" alt="${m.name}"></div>
                    <div style="flex:1;">
...
function publishStudioMeme() {
    alert("🚀 Custom meme published to For You Page!");
    playMagicSound('victory');
}
...
// Background Particle & Sound Synthesizer
let audioCtx = null;
